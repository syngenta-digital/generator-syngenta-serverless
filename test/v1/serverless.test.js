                ...
                describe('#sns', () => {
                    ...
                    it('topic resource created properly', () => {
                        return new Promise(async resolve => {
                            ...
                            const { Resources } = read_resource;
                            assert.notEqual(Resources, undefined);
                            assert.notEqual(Resources[`${topic_name}Topic`], undefined);
                            assert.equal(JSON.stringify(Resources[`${topic_name}Topic`]), JSON.stringify(topic_template));
                            resolve();
                        });
                    });
                    it('topic subscription created properly', () => {
                        return new Promise(async resolve => {
                            ...
                            const { Resources } = read_resource;
                            assert.notEqual(Resources, undefined);
                            assert.notEqual(Resources[`${topic_name}Subscription`], undefined);
                            assert.equal(JSON.stringify(Resources[`${topic_name}Subscription`]), JSON.stringify(subscription_template));
                            resolve();
                        });
                    });
                    it('iam role created properly', () => {
                ...
                describe('#s3', () => {
                    ...
                    it('resource created properly', () => {
                        return new Promise(async resolve => {
                            ...
                            const { Resources } = read_resource;
                            assert.notEqual(Resources, undefined);
                            assert.notEqual(Resources[`${bucket_name}Storage`], undefined);
                            assert.equal(JSON.stringify(Resources[`${bucket_name}Storage`]), JSON.stringify(bucket_template));
                            assert.equal(JSON.stringify(Resources[`AttachmentsBucketAllowPublicReadPolicy${bucket_name}`]), JSON.stringify(public_policy_template));
                            resolve();
                        });
                    });
                    it('iam role created properly', () => {
